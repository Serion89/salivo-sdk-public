...
## 8. Developer Tooling & VS Code Extension (v1.0.8)

The official Salivo Visual Studio Code extension provides first-class language support and is installed directly at `~/.vscode/extensions/salivo.salivo-1.0.8`:

- **Sub-30ms Warm Execution (`ExecCache`)**: Press `Ctrl + F5` or click the title bar Play button to execute `.sal` files instantly using warm binary hash caching.
- **Standalone Binary Generation**: Press `Ctrl + Shift + B` to compile the active file into an optimized standalone `.exe` (`sf build`).
- **Rapid Semantic Validation**: Press `Ctrl + Shift + K` to run full type checking and syntax validation without object code emission (`sf check`).
- **Document Formatting**: Press `Shift + Alt + F` to invoke `salivofmt --stdin`.
- **Integrated Static Analysis**: Press `Ctrl + Shift + L` to run `salivolint`.
- **Package Testing**: Execute `spm test` directly from editor context menus.
- **Custom Toolchain Configuration**: Configurable in VS Code settings under `salivo.executablePath`, `salivo.formatterPath`, `salivo.linterPath`, and `salivo.packageManagerPath`.
- **Streamlined Snippets**: Includes built-in snippets for `main`, `func`, `pubfunc`, `test`, `struct`, `enum`, `trait`, `impl`, `drop`, `for` (3-clause), `while`, `try` (`?`), `match`, `importcore`, `importfs`, `vec`, `map`, `stringnew`, `sqlite`, and `webrouter`.
